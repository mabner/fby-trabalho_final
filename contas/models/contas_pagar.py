...
class PagarQuerySet(models.QuerySet):
	...
	# Query para retornar o valor total das contas pagas
	def total_pago(self):
		return float(self.filter(situacao="P").aggregate(
				Sum('valor'))['valor__sum'])
...
class PagarManager(models.Manager):
	...
	# Retornar a soma das contas pagas
	def obter_total_pago(self):
		return self.get_queryset().total_pago()

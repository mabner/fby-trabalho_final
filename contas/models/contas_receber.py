...
class ReceberQuerySet(models.QuerySet):
	...
	# Query para retornar o valor total de contas recebidas
	def total_recebido(self):
		return float(self.filter(situacao="R").aggregate(
				Sum('valor'))['valor__sum'])
...
class ReceberManager(models.Manager):
	...
	# Retornar a soma das contas recebidas
	def obter_total_recebido(self):
		return self.get_queryset().total_recebido()
